...
class MyApp(QWidget):
    ...
    def slidersChanchedH(self):
        self.inputH.setText(str(round(float(self.sliderH.value() / 800), 2)))

    def slidersChanchedL(self):
        self.inputL.setText(str(round(float(self.sliderL.value() / 800), 2)))

    def slidersChanchedS(self):
        self.inputS.setText(str(round(float(self.sliderS.value() / 800), 2)))

    def slidersChanchedC(self):
        self.inputC.setText(str(round(float(self.sliderC.value() / 800), 2)))

    def slidersChanchedM(self):
        self.inputM.setText(str(round(float(self.sliderM.value() / 800), 2)))

    def slidersChanchedY(self):
        self.inputY.setText(str(round(float(self.sliderY.value() / 800), 2)))

    def slidersChanchedK(self):
        self.inputK.setText(str(round(float(self.sliderK.value() / 800), 2)))
    ...
    def updateButtonColorHLS(self):
        ...
        self.inputH.setText(str(round(float(h_text), 2)))
        self.inputL.setText(str(round(float(l_text), 2)))
        self.inputS.setText(str(round(float(s_text), 2)))

        if len(h_text) == 2 and h_text[1] == '.':
            h_text = '0.'
            self.inputH.setText(str(h_text))
        if len(l_text) == 2 and l_text[1] == '.':
            l_text = '0.'
            self.inputL.setText(str(l_text))
        if len(s_text) == 2 and s_text[1] == '.':
            s_text = '0.'
            self.inputS.setText(str(s_text))
    ...
    def updateButtonColorCMYK(self):
        ...
        self.inputC.setText(str(round(float(c_text), 2)))
        self.inputM.setText(str(round(float(m_text), 2)))
        self.inputY.setText(str(round(float(y_text), 2)))
        self.inputK.setText(str(round(float(k_text), 2)))

        if len(c_text) == 2 and c_text[1] == '.':
            c_text = '0.'
            self.inputC.setText(str(c_text))

        if len(m_text) == 2 and m_text[1] == '.':
            m_text = '0.'
            self.inputM.setText(str(m_text))

        if len(y_text) == 2 and y_text[1] == '.':
            y_text = '0.'
            self.inputY.setText(str(y_text))

        if len(k_text) == 2 and k_text[1] == '.':
            k_text = '0.'
            self.inputK.setText(str(k_text))

        color = QColor.fromCmykF(round(float(c_text), 2), round(float(m_text), 2), round(float(y_text), 2), round(float(k_text), 2))
    ...
    def changeColor(self, color):
        ...
        if not self.inputHLS:
            ...
            if h == 0.0:
                ...
                self.inputH.setText(str(round(h, 2)))

            if l == 0.0:
                self.inputL.setText('0.')
            else:
                self.inputL.setText(str(round(l, 2)))

            if s == 0.0:
                self.inputS.setText('0.')
            else:
                self.inputS.setText(str(round(s, 2)))

        if not self.inputCMYK:
            c, m, y, k, _ = color.getCmykF()

            self.inputC.setText(str(round(c, 2)))
            self.inputM.setText(str(round(m, 2)))
            self.inputY.setText(str(round(y, 2)))
            self.inputK.setText(str(round(k, 2)))
        ...
        self.sliderH.setValue(int(round(float(self.inputH.text()) * 800, 2)))
        self.sliderL.setValue(int(round(float(self.inputL.text()) * 800, 2)))
        self.sliderS.setValue(int(round(float(self.inputS.text()) * 800, 2)))
        self.sliderC.setValue(int(round(float(self.inputC.text()) * 800, 2)))
        self.sliderM.setValue(int(round(float(self.inputM.text()) * 800, 2)))
        self.sliderY.setValue(int(round(float(self.inputY.text()) * 800, 2)))
        self.sliderK.setValue(int(round(float(self.inputK.text()) * 800, 2)))
